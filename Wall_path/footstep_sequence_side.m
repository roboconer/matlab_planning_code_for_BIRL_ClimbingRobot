function [ se_footstep3d ] = footstep_sequence_side( pathstr,map_border,obs_border,obs_height )
...
if norm(ciro-pathstr(num_pa,:))<r_min              %起始点和目标点之间小于一步可到达的最小距离
     ...
     d1 = -30;
     d2 = -60;
     rotationMatrix1 = [cosd(d1), -sind(d1); sind(d1), cosd(d1)]; %90°
%      rotationMatrix2 = [cosd(d2), -sind(d2); sind(d2), cosd(d2)]; %-90°
...
%      end
     ...
     [foot_str] = footstep_strategy(apathline3);
%      [foot_str2] = footstep_strategy(apathline4);
%      num_f1 = size(foot_str,1);
%      for i=1:size(foot_str2,1)
%          foot_str(num_f1+i,:) = foot_str2(i,:);
%      end
