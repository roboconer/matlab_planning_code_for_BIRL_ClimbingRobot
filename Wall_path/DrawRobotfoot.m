function [] = DrawRobotfoot(se_footstep3d,Vn)
...
%[tr,an]= robot_configuration(x);  %画多壁面全局路径的机器人构型
[tr,an]= robot_configurationw(se_footstep3d,Vn);   %画单一壁面机器人多步态构型
...
for i=1:num_robot
    ...
    Climbot5D.CurGraspMtx(1:3,4) = Climbot5D.CurGraspMtx(1:3,4)*1000;%1000*x(2*i-1,:)';
    Climbot5D.TarJAs(1:4) = an(i,1:4);
     DrawRobot(Climbot5D,1);
end

  
%     Climbot5D.CurGraspMtx = tr(:,:,1);
%     Climbot5D.CurGraspMtx(1:3,4) = Climbot5D.CurGraspMtx(1:3,4)*1000;
%     Climbot5D.TarJAs(2:4) = an(1,2:4);
%      DrawRobot(Climbot5D,1);
