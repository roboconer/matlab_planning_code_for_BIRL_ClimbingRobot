...
% ======== 以下开始移动机器人的各个零部件 ==========
global BandClamp HollowPart IModule GModule;

% 移动到夹持器原点准备开始画图
if robot.FixedGripper == 2
    
    DrawCor = robot.CurGraspMtx*tl(0,0,179.5);
    % 第一个夹持器
    UpdateVs(hR(1),DrawCor,GModule.vertices');
    % 卡环
...
else   % 如果FixedGripper == 1，第一个T模块之前的都不需要动
    
    DrawCor = robot.CurGraspMtx*tl(0,0,342.9)*rz(JAs(1))*tl(0,0,93.4)*ry(-JAs(2))*tl(0,0,146.6);
...
% 第二个夹持器
UpdateVs(hR(18),DrawCor*rz(JAs(JANum+1))*ry(180),GModule.vertices');
