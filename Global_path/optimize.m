...
% %     f(num_f+i)=norm(Ptar(i,:))-0.586+0.002;             %两个过渡吸附点之间的位置约束
% %     ceq(num_ceq+i)=dot(Ptr(2*i,:)-Ptr(2*i-1,:),Vy(2*i-1,:));  %必须在前一个坐标系的y=0平面上
% 
%       f(num_f+ 2 * i - 1)=norm(Ptar(i,:))-0.586+0.002;             %两个过渡吸附点之间的位置约束
%       f(num_f+ 2 * i)=dot(Ptr(2*i,:)-Ptr(2*i-1,:),Vy(2*i-1,:)) - 0.001;  %必须在前一个坐标系的y=0平面上
% end
% num_f = num_f + 2 * (num_sur-1);
...
for i=1:num_sur-1
    f(num_f+i)=norm(Ptar(i,:))-0.50+0.002;             %两个过渡吸附点之间的位置约束
...
%  if norm(x(1,:)-Pstart)<0.4
%      ceq(num_ceq +1) = norm(x(1,:)-Pstart);
%      num_ceq = num_ceq +1;
%  end
%  if norm(x(2*(num_sur-1),:)-Pend)<0.4
%      ceq(num_ceq +1) = norm(x(2*(num_sur-1),:)-Pend);
%      num_ceq = num_ceq +1;
%  end
%  if num_sur >2
%  for i = 1:num_sur-2
%      if norm(x(2*i,:)-x(2*i+1,:))<0.4

%        f(num_f + 1) = 0.4-norm(x(2,:)-x(3,:));
%       num_f = num_f +1;
%    f(num_f+1) = 0.4-norm(x(4,:)-x(5,:));
%      num_f = num_f +1;
%      f(num_f+1) = 0.4-norm(x(6,:)-[1.1388    1.8585    2.0091]);
%      num_f = num_f +1;
      
      %ceq(num_ceq+1) = norm(x(1,:)-[0.4500    0.4986    0.8038]);
    % ceq(num_ceq+1) = norm(x(1,:)-[-0.2045 -0.0595 0]);
    % num_ceq = num_ceq +1;
