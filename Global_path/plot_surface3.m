...
%color=[7 6 5 3 2];
for i=1:num_sur
  % pp = patch(A{i}(:,1),A{i}(:,2),A{i}(:,3),[0.467,0.533,0.6],'facealpha',0.5);
   % pp = patch(A{i}(:,1)+1000,A{i}(:,2)-1000,A{i}(:,3),50+15*i,'facealpha',0.5);
   pp = patch(A{i}(:,1),A{i}(:,2),A{i}(:,3),[0.467,0.533,0.6],'facealpha',0.5,'Linestyle','- -','LineWidth',1);
   % pp.FaceColor='blue';
    hold on
end
...
for i=1:num_sur
    ...
    mArrow3([A{i}(2,1),A{i}(2,2),A{i}(2,3)],[Vn(i,1),Vn(i,2),Vn(i,3)]*0.7+[A{i}(2,1),A{i}(2,2),A{i}(2,3)],'color','m','stemWidth',20,'facealpha',0.7);
end
...
fz =17;
%hxl= xlabel('\it{X}\rm(\it{m}\rm)','FontSize',fz,'FontName','Times New Roman');        %设置坐标轴格式
...
%set(hzl,'Position',[-1100 -1200 5000])
