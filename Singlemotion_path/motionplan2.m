...
%function [ Ba,Ba_R,joint_ang] = motionplan2()
%单步运动规划求解
...
for i=1:num_surfaces
    ...
    l = r/ang_sin;
    expand_surface{i}(1,:) = surface{i}(1,:)+ v1*l +v2*l;
    ...
    ang_sin = sqrt(1-ang_cos^2);
    l = r/ang_sin;
    expand_surface{i}(num_point_each_surface(i),:) = surface{i}(num_point_each_surface(i),:)+ v1*l +v2*l;
   ...
   for j=2:num_point_each_surface(i)-1
       ...
       ang_sin = sqrt(1-ang_cos^2);
       l = r/ang_sin;
       expand_surface{i}(j,:) = surface{i}(j,:)+ v1*l +v2*l;   
   end
end
...
for j = 1:size(wall_planning_points,1) - 2
    ...
    for i = 1:num_surfaces%求解机器人工作空间内的有效障碍物
        ...
        obs_p(num_obstructs*8-7:(num_obstructs + num_ob2 - 1)*8,:)  = map_obs( expand_surface{i},obstruct_borders{i},obstruct_height{i} );
        % obs_p(1:8,:) = map_obs( expand_surface{i},obstruct_borders{i},obstruct_height{i} );
