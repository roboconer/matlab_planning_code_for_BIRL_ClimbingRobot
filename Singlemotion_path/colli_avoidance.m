...
dis = 10000;
TO_SURFACE_DISTANCE = 0.15; %单步规划时，吸盘抬起的距离
vec_lin = zeros(1,3);   
vec_eff = zeros(1,3);
...
if DRAW
  ...
  %画图范围设置 by:CH
  set(gca,'xlim',[-1000,3000])%将x轴上的取值范围设置为[-100,1500] 
  xlabel('x');
  set(gca,'ylim',[-1000,3000])%将y轴上的取值范围设置为[-1500,1500] 
  ylabel('y');
  set(gca,'zlim',[0,3000])%将z轴上的取值范围设置为[200,1500] 
  zlabel('z');
  hold on 
end
...
%%求解起始点目标点在同一壁面的情形
%//////////////////////////////// 1）先让机器人上升TO_SURFACE_DISTANCE * 100cm,对起始点和目标点各提升一段距离用作平缓靠近壁面 
...
if CREATE_G_CODE ~= 1 %生成G代码的话要把下降过程去掉
...
% joint_ang(:,5) = 0;
% if CREATE_G_CODE == 1 %如果要生成自主吸附的G代码，要在关节角序列的最后将第五关节清零
%     joint_ang(num_ang,5) = 0;
% %     tmp_joint_angle = [joint_ang(num_ang,1:4), 0];
% %     joint_ang = [joint_ang;tmp_joint_angle];
% %     num_ang = num_ang + 1;
% end
...
%         wrobot.TarGraspMtx(1:3,1:3) = stp_R;
%         wrobot.TarGraspMtx(1:3,4) = enp'*1000;        %求机器人目标构型的关节角
%         T = wrobot.TarGraspMtx;
%     end
    
  
   T(1:3,2) = T(1:3,2)*-1;  %Y,Z轴转了过来的T_base_end
   T(1:3,3) = T(1:3,3)*-1;
  lastbas_R = bas_R*T(1:3,1:3);
   laststp_R = T(1:3,1:3)';  %laststp_R是R_end_base
