...
stp_o = stp;
bap_o = bap;
TO_SURFACE_DISTANCE = 0.15;%离壁面的距离
...
jmax = [360,210,120,210,360];    %关节角变化的最大值
jmin = [-360,-30,-120,-30,-360];  %关节角变化的最小值
...
enpp =enp;
enp = enp + (wor_to_bas_R*Vnm(2,:)')'*TO_SURFACE_DISTANCE;       %目标吸附点的辅助点
...
un_sta = sta_ang(1,5); %记录活动端的I关节角度
wrobot.TarGraspMtx(1:3,4) = enpp'*1000;        %求机器人目标构型的关节角
...
for i = 1:TO_SURFACE_DISTANCE * 100             %先让机器人上升5cm,并对起始点和目标点各提升一段距离用作平缓靠近壁面 
...
while norm(joint_ang(num_ang,:)-end_ang)>1e-4
    ...
    if norm(joi_p(4,:))<0.4  %末端距离基座过近
        ...
        for i = 1:5             %判断规划得到的关节角值有没超出允许范围，超出则保留为上一状态的值
...
disp('下降过程规划')
%  if CREATE_G_CODE ~= 1
%      for i = 1:(TO_SURFACE_DISTANCE-0.01) * 100-1             %使机器人末端点往Z轴方向靠近TO_SURFACE_DISTANCE-0.01，每次1cm
...
%     for i = 1:2               %使机器人末端点往Z轴方向靠近1cm，每次0.5cm
%           [ T,end_p] = Kine5D( joint_ang(num_ang-1+i,:) ,4);
...
%  end
 ...
 if CREATE_G_CODE ~= 1
     for i = 1:TO_SURFACE_DISTANCE * 100             %使机器人末端点往Z轴方向靠近TO_SURFACE_DISTANCE-0.01，每次1cm
          [ T,end_p] = Kine5D( joint_ang(num_ang-1+i,:) ,4);
          end_p = end_p - (wor_to_bas_R*Vnm(2,:)')'*0.01*1000;
          wrobot.TarGraspMtx(1:3,4) = end_p';
          wrobot.CurJAs=joint_ang(num_ang+i-1,:);
