...
vec = cross(poly(2,:)-poly(1,:),poly(3,:)-poly(2,:));  % 法向量垂直面向里(逆时针定序)
vec = vec/norm(vec);
...
function [state] = insidepoly3(point,poly)        %判断点是否在空间多边形内部(点不在面内，则返回0)
num_p=size(poly,1);
mean_p=zeros(1,3);
kk=0;
for i=1:num_p
...
function[obs_sur] =obp_tran(obp)%将代表障碍物的8个三维空间点转化为障碍物元素（5个面元素和12个线段元素）
    num_ob = size(obp,1)/8;
