function [pathstr,path_dis] = path_straight( path )
%UNTITLED 此处显示有关此函数的摘要
%   将粒子群全局路径拉直
[ map_bor,acr_obs,att_obs,byp_obs ] = map; 
   ...
   [inter_l1,byp_stm2(j)] =linepolyintersection(l1,byp1_obs(j*4-3:j*4,:),2);
      if byp_stm2(j)>=1
       ...
       for j=1:k_att             
        ...
        for j=1:k_att             
                 att_stm1(j+k_att) = insidepolygon(att2_obs(j*4-3:j*4,:),l1(2,:));
                 if att_stm1(j+k_att)==1
                     break;
                 end
       ...
       if all(byp_stm1<1)&&all(byp_stm2<1)&&all(acr_stm1<1)&&all(acr_stm2<2)&&all(att_stm1(k_att:2*k_att)<1)&&all(att_stm2<2)||any(att_stm1(1:k_att)==1)
           continue;
       else
          % if i+k-s>1
...
plot(pathstr(:,1)*1000,pathstr(:,2)*1000,'-or','LineWidth',2);
end
