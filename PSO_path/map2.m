function [ map_bor,acr_obs,att_obs,byp_obs,hei_obs ] = map2( map_border,obs_border,obs_height )
%MAP 此处显示有关此函数的摘要
...
for i=1:num_obs                                                    %对障碍物进行分类
    if obs_wid(i)>0.4 && obs_height(i)<0.40                      %判断机器人能否吸附到障碍物上，高度小于0.428m
        n_att=n_att+1;
        k_att(n_att)=i;
        continue;
    end
...
for i=1:n_byp
    hei_obs(i) = obs_height(k_byp(i));    %先记录必须绕过的障碍物高度
end
for i =1:n_acr
    hei_obs(n_byp+i) = obs_height(k_acr(i));     %再记录可以跨越的障碍物高度
end
for i=1:n_att
    hei_obs(n_byp+n_acr+i) = obs_height(k_att(i));     %最后记录可以吸附的障碍物高度
end
