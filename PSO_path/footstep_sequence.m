function [ se_footstep3d ] = footstep_sequence( pathstr )
%UNTITLED 此处显示有关此函数的摘要
%   根据粒子群全局路径与爬壁机器人步距范围输出离散的落足点，利用有限种步态策略搜索与评估方法
global foot;
tic
foot= footgen(0.04,10,60);   %根据参数生成落足点策略，第一个参数为距离间隔，第二个为角度间隔，第三个为角度范围（包括顺逆时针）
r_max=0.58;
r_min=0.4;
...
in_bor = insidepolygon(map_bor,apathline(2,:));
 for j=1:k_byp                                
   ...
   for j=1:k_acr
           ...
           [inter_l1,acr_stm2(j)] =linepolyintersection(apathline,acr1_obs(j*4-3:j*4,:),2);
           [inter_l1,acr_stm2(k_acr+j)] =linepolyintersection(apathline,acr2_obs(j*4-3:j*4,:),2);
           [inter_l1,acr_stm2(2*k_acr+j)] =linepolyintersection(apathline,acr3_obs(j*4-3:j*4,:),2);

    end
       for j=1:k_att             
                 att_stm1(j) = insidepolygon(att3_obs(j*4-3:j*4,:),apathline(2,:));
                 if att_stm1(j)==1
                     break;
                 end
                 att_stm1(k_att+j) = insidepolygon(att2_obs(j*4-3:j*4,:),apathline(2,:));
                 if att_stm1(k_att+j)==1&&att_stm1(j)==0
                     break;
                 end
           [inter_l1,att_stm2(j)] =linepolyintersection(apathline,att1_obs(j*4-3:j*4,:),2);
               if att_stm2(j)==2
                     break;
               end
            [inter_l1,att_stm2(k_att+j)] =linepolyintersection(apathline,att3_obs(j*4-3:j*4,:),2);
               if att_stm2(k_att+j)==2
                     break;
               end
               [inter_l1,att_stm2(2*k_att+j)] =linepolyintersection(apathline,att2_obs(j*4-3:j*4,:),2); 
       end
      if in_bor==0||any(byp_stm1==1)||any(byp_stm2==2)||any(acr_stm1==1)||any(att_stm1(k_att+1:2*k_att))==1||any(att_stm2(1:2*k_att)==2)
          state_judge=0;
          return;
      end
       
      if any(acr_stm2(1:k_acr)==2)
          for i=1:k_acr
              if acr_stm2(i)==2
                  hei = hei_obs(k_byp+i);
...
%                    acr_len=norm(inter_l1(1,:)-inter_l1(2,:));
      ...
      elseif any(acr_stm2(k_acr+1:2*k_acr)==2)
          for i=1:k_acr
              if acr_stm2(k_acr+i)==2
                 if acr_stm2(2*k_acr+i)<2
                     state_judge = 1;
                     return;
                 else
                     state_judge = 0;
      ...
      elseif any(att_stm2(2*k_att:3*k_att)==2)
             state_judge =1;
             return;
      elseif foot_hei(1)>0
           hei = abs(foot_hei(1)-foot_hei(2)); 
...
if norm(ciro-pathstr(num_pa,:))<r_min              %起始点和目标点之间小于一步可到达的最小距离
     ...
     for i=1:size(foot_str2,1)
         foot_str(size(foot_str,1)+i,:) = foot_str2(i,:);
     end
...
%当一步即可到达目标点的情况处理或者剩最后一步即可到达目标点的情况处理
 ...
 elseif last_num ==0 && norm(se_footstep(num_step,:)-pathstr(num_pa,:))>=r_min    %起始点和目标点在一步运动范围内但不能直接一步到达的情况，看情况判断是否需要添加步数
     ...
     if state ==0
         h=msgbox('Sorry, No path exists to the Target for the first or last step!','warn');
         uiwait(h,3);
     end 
 elseif last_num>0
    for j = 1: last_num-1
